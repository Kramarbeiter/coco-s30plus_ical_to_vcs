...
class Event:
    ...
    def translate_and_build_summary(self):
        ...
        if len(test_summary) > 40 and location:
            print(
                f"  -> WARNING: Location '{location}' in '{title[:15]}...' skipped (Subject otherwise > 40 chars)."
            )
            location = ""
            test_summary = assemble(title, location, logic_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all", action="store_true", help="Export all events, including past ones"
    )
    args = parser.parse_args()

    # 1. Find all .ics files in the current directory
    ics_files = sorted(
        [f for f in os.listdir(".") if os.path.isfile(f) and f.lower().endswith(".ics")]
    )
    ...
    file_selection = input(
        "\nWhich files should be processed? (Enter single number / multiple comma-seperated numbers or just press Enter to process all files): "
    ).strip()
    ...
    for current_file in selected_files:
        ...
        user_input = input(
            f"How many events shall be processed? (1-{total}, or Enter for all): "
        ).strip()
        ...
        print(
            f"  -> {export_count} events from '{current_file}' created in '{out_dir}'."
        )
